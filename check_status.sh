...
PREVIEW_FRONTEND=$(curl -s https://login-retry.preview.emergentagent.com/ 2>&1 | grep -o "Pinspire")
...
PREVIEW_BACKEND=$(curl -s https://login-retry.preview.emergentagent.com/api/auth/login \
